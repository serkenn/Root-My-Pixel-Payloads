...
int spawn_root_child(void) {
  ...
  if (root_child_pid == 0) {
    ...
    if (!atomic_load(&root_shared->go)) {
      pr_error("root child go timeout uid_before=%u\n",
               (unsigned)captured_uid_before);
      _exit(2);
    }
    ...
    errno = 0;
    pr_info("root child step setgid uid_before=%u\n",
            (unsigned)captured_uid_before);
    report.setgid_ret = setgid(0);
    report.setgid_errno = errno;
    errno = 0;
    pr_info("root child step setuid entering (setgid_ret=%d)\n",
            report.setgid_ret);
    ...
    pr_info("root child step post-setuid uid=%u euid=%u gid=%u egid=%u "
            "setgid_ret=%d setuid_ret=%d\n",
            (unsigned)report.uid_after, (unsigned)report.euid_after,
            (unsigned)report.gid_after, (unsigned)report.egid_after,
            report.setgid_ret, report.setuid_ret);
    ...
    if (report.setgid_ret == 0 && report.setuid_ret == 0) {
      ...
      pr_info("root child step su install done ret=%d errno=%d daemon=%d\n",
              report.su_install_ret, report.su_install_errno,
              (int)report.su_daemon_pid);
      errno = 0;
      report.wallpaper_ret = install_embedded_wallpaper();
      report.wallpaper_errno = errno;
    } else {
      pr_warning("root child step setgid/setuid failed gid=%d uid=%d\n",
                 report.setgid_ret, report.setuid_ret);
    ...
    pr_info("root child step report done uid_after=%u\n",
            (unsigned)report.uid_after);
...
int collect_root_child(void) {
  ...
  if (!atomic_load(&root_shared->done)) {
    pr_warning("root child done timeout pid=%d\n", (int)root_child_pid);
    return 0;
  }
