...
{
    /* The kernel sizes the futex hash table with
     * roundup_pow_of_two(256 * num_possible_cpus()). Using only the online
     * count and no rounding produces a different mask on CPUs whose
     * nproc*256 is not a power of two (e.g. 9 cores -> 2304 vs 4096),
     * which makes the mm_struct brute-force always miss. */
    ...
    unsigned long size = (unsigned long)nproc * 256;
    unsigned long rounded = 1;
    while (rounded < size) {
        rounded <<= 1;
    }
    futex_hashsize = rounded;
}
